...
pause:
		...
		if (should_test) {
			switch (dsa_emu_debug_origin_bdp_mode) {
			case 0: // normal
			...
			case 1: // no_wb
				break;
			case 2: {
				long nr_pages = LONG_MAX;
				if (dsa_emu_debug_origin_bdp_fg_wb_nrpages !=
				    0) {
					nr_pages =
				...
				wb_writeback(wb, &work, true);
				break;
			}
			default:
				pr_warn("Unknown dsa_emu_debug_origin_bdp_mode: %d\n",
					dsa_emu_debug_origin_bdp_mode);
				break;
			}
			break;
		} else {
...
EXPORT_SYMBOL(folio_account_dirtied);
