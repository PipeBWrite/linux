		...
		unsigned long nr_to_read, unsigned long lookahead_size)
{
	...
	struct ongoing_list_bucket *olist_buckets = NULL;

	if (inode && filemap_bg_compatible(inode))
		olist_buckets = READ_ONCE(inode->i_ongoing_buckets);
	...
	for (i = 0; i < nr_to_read; i++) {
		...
		/*
		 * Skip pages with pending BG writes.  The BG path will
		 * add the folio to the page cache shortly; adding a
		 * duplicate here would cause BG's filemap_add_folio_test()
		 * to fail with EEXIST.
		 */
		if (olist_buckets &&
		    search_ongoing_entry_state(index + i, olist_buckets)) {
			read_pages(ractl);
			ractl->_index++;
			i = ractl->_index + ractl->_nr_pages - index - 1;
			continue;
		...
		struct file_ra_state *ra, unsigned int new_order)
{
	...
	while (index <= limit) {
		...
		while (index + (1UL << order) - 1 > limit)
			order--;
		/* Check o-list; bail to fallback (page_cache_ra_unbounded) */
		if (olist_buckets) {
			pgoff_t j;

			for (j = index; j < index + (1UL << order); j++) {
				if (search_ongoing_entry_state(j, olist_buckets)) {
					err = -EEXIST;
					break;
				}
			}
