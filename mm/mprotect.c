...
mprotect_fixup(struct vma_iterator *vmi, struct mmu_gather *tlb,
	       ...
	       unsigned long start, unsigned long end, unsigned long newflags, bool dsa)
{
	struct mm_struct *mm = vma->vm_mm;
	...
	if ((newflags == oldflags) && (vma->dsa_protected == dsa)) {
		*pprev = vma;
		return 0;
	}
	...
	*pprev = vma;
	...
	vma_set_page_prot(vma, dsa);
	vma->dsa_protected = dsa;
...
int do_mprotect_pkey(unsigned long start, size_t len,
		unsigned long prot, int pkey, bool dsa)
{
	...
	for_each_vma_range(vmi, vma, end) {
		...
		new_vma_pkey = arch_override_mprotect_pkey(vma, prot, pkey); // -1
		newflags = calc_vm_prot_bits(prot, new_vma_pkey); // Translates to VM_* FLAGS
		...
		error = mprotect_fixup(&vmi, &tlb, vma, &prev, nstart, tmp, newflags, dsa);
		if (error)
			break;
...
SYSCALL_DEFINE3(mprotect, unsigned long, start, size_t, len,
		unsigned long, prot)
{
	return do_mprotect_pkey(start, len, prot, -1, 0);
}

SYSCALL_DEFINE3(mprotect_l, unsigned long, start, size_t, len,
		unsigned long, prot)
{
	if (prot == PROT_NONE) {
		prot = PROT_READ | PROT_WRITE;
		return do_mprotect_pkey(start, len, prot, -1, 1);
	} else {
		return do_mprotect_pkey(start, len, prot, -1, 0);
	}
}
...
{
	return do_mprotect_pkey(start, len, prot, pkey, 0);
}
