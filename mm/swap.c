...
{
#ifdef CONFIG_SCACHE_MEMORY
	if (unlikely(PageScache(&folio->page))) {
		page_cache_release(folio);
		mem_cgroup_uncharge(folio);
		if (sc_pool.enabled) {
			/* recycle into the pool (clears PG_scache, count==1) */
			sc_release_page(&folio->page);
		} else {
			/* pool gone: clear the flag so the page is a normal
			 * buddy page again (avoids CHECK_AT_PREP on realloc) */
			ClearPageScache(&folio->page);
...
void folios_put_refs(struct folio_batch *folios, unsigned int *refs)
{
	...
	unsigned long flags = 0;
#ifdef CONFIG_SCACHE_MEMORY
	struct folio_batch sc_batch;

	folio_batch_init(&sc_batch);
#endif
...
#ifdef CONFIG_SCACHE_MEMORY
		if (unlikely(PageScache(&folio->page)) && !folio_test_large(folio)) {
			if (sc_pool.enabled) {
				/* recycle after the lruvec lock is dropped */
				folio_batch_add(&sc_batch, folio);
				continue;
			}
			/* pool gone: drop the flag, free as a normal page */
			ClearPageScache(&folio->page);
		}
#endif
	...
	if (lruvec)
		unlock_page_lruvec_irqrestore(lruvec, flags);
#ifdef CONFIG_SCACHE_MEMORY
	if (folio_batch_count(&sc_batch)) {
		mem_cgroup_uncharge_folios(&sc_batch);
		for (i = 0; i < folio_batch_count(&sc_batch); i++)
			sc_release_page(&sc_batch.folios[i]->page);
	}
#endif
	if (!j) {
