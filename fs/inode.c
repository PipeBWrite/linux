...
int inode_init_always(struct super_block *sb, struct inode *inode)
{
	...
	atomic_set(&inode->i_bg_pending, 0);
	inode->i_fg_batch = NULL;
	init_completion(&inode->i_bg_drained);
	complete_all(&inode->i_bg_drained);  /* Start in completed state */
	...
	sc_per_file_cache_init(&inode->i_sc_cache,
			       SC_LOW_MARK_DEFAULT, SC_HIGH_MARK_DEFAULT);
#endif
...
static struct inode *alloc_inode(struct super_block *sb)
{
	...
	inode->i_ongoing_buckets = kcalloc(ONGOING_HASH_NUM,
					   sizeof(*inode->i_ongoing_buckets),
					   GFP_KERNEL);
	atomic_set(&inode->i_draining, 0);
	inode->i_snapshot_counter = DSA_EMU_BG_SNAPSHOT_COUNTER;
	inode->i_bg_ongoing_snapshot = 0;
	for (int i = 0; i < ONGOING_HASH_NUM; i++)
		INIT_HLIST_HEAD(&inode->i_ongoing_buckets[i].list);
...
static void destroy_inode(struct inode *inode)
{
	...
	/*
	 * Note: For filesystems with destroy_inode but no free_inode (like XFS),
	 * the early return above means this code won't run - the filesystem
	 * is responsible for freeing these resources.
	 */
	if (inode->i_ongoing_buckets)
		kfree(inode->i_ongoing_buckets);
	if (inode->i_fg_batch)
		kfree(inode->i_fg_batch);
...
void inode_init_once(struct inode *inode)
{
	...
	// spin_lock_init(&inode->i_draining_lock);
	spin_lock_init(&inode->i_shift_index_lock);
...
#ifdef CONFIG_SCACHE_MEMORY
	if (inode->i_sc_cache.nr_free)
		sc_per_file_cache_destroy(&inode->i_sc_cache, &sc_pool);
#endif
