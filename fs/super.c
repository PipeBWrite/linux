...
#ifdef CONFIG_INTEL_DSA_SL_EMU
		/*
		 * Drop FG batched refs and wait BG completions before eviction.
		 * Otherwise, tiny-write batches can pin inodes across unmount.
		 */
	again:
		spin_lock(&sb->s_inode_list_lock);
		list_for_each_entry(inode, &sb->s_inodes, i_sb_list) {
			if (!(inode->i_fg_batch && READ_ONCE(inode->i_fg_batch->nr_reqs)) &&
			    !atomic_read(&inode->i_bg_pending))
				continue;
			if (!igrab(inode))
				continue;
