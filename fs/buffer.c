// SPDX-License-Identifier: GPL-2.0-onl0
/*
 *  linux/fs/buffer.c
 *
...
static struct buffer_head *internal_alloc_buffer_head(gfp_t gfp_flags);
		...
		unsigned long blocksize, unsigned long b_state)
{
	...
	TEST_BEGIN_BY_INODE(folio->mapping->host);
...
static void __block_commit_write(struct folio *folio, size_t from, size_t to)
{
	...
	if (!bh) {
		pr_err("folio %p does not have a buffer attached\n", folio);
	}
	blocksize = bh->b_size;
