		    ...
		    struct ext4_map_blocks *map, int flags)
{
	 ...
	 * file system block.
	 */
	DOWN_READ_TEST(inode, &EXT4_I(inode)->i_data_sem);
	if (ext4_test_inode_flag(inode, EXT4_INODE_EXTENTS)) {
		retval = ext4_ext_map_blocks(handle, inode, map, 0);
	} else {
		retval = ext4_ind_map_blocks(handle, inode, map, 0);
	...
	UP_READ_TEST(inode, (&EXT4_I(inode)->i_data_sem));
...
found:
	 ...
	 * with create == 1 flag.
	 */
	DOWN_WRITE_TEST(inode, &EXT4_I(inode)->i_data_sem); // MAYBE HERE?
...
out_sem:
	UP_WRITE_TEST(inode, (&EXT4_I(inode)->i_data_sem));
	if (retval > 0 && map->m_flags & EXT4_MAP_MAPPED) {
		ret = check_block_validity(inode, map);
		if (ret != 0)
			return ret;
...
int ext4_block_write_begin_for_test(loff_t pos, struct folio *folio, unsigned len,
				  get_block_t *get_block)
{
	...
	BUG_ON(to > PAGE_SIZE);
	BUG_ON(from > to);
	TEST_BEGIN_BY_INODE(inode);

	head = folio_buffers(folio);
	if (!head) {
		// BG_ADD, BG_END, TIME: 95.48 -> 512.33
...
EXPORT_SYMBOL(ext4_block_write_begin_for_test);
				  ...
				  get_block_t *get_block)
{
	...
	if (!folio_test_locked(folio)) {
		pr_err("folio %p is not locked\n", folio);
		struct folio *another_folio = filemap_get_folio(folio->mapping, folio->index);
		if (!IS_ERR(another_folio)) {
			if (folio_test_locked(another_folio)) {
				pr_err("another folio %p is locked\n", another_folio);
			} else {
				pr_err("another folio %p is not locked\n", another_folio);
			}
		} else {
			pr_err("failed to get another folio\n");
		}
		dump_stack();
	}
...
static int ext4_block_write_begin_bg(struct folio *folio, loff_t pos, unsigned len)
{
	return ext4_block_write_begin(folio, pos, len, ext4_da_get_block_prep);
}
#endif
				       ...
				       bool invalidate)
{
	...
	if (invalidate) {
		 ...
		 */
		DOWN_WRITE_TEST(inode, &EXT4_I(inode)->i_data_sem);
...
static int mpage_prepare_extent_to_map(struct mpage_da_data *mpd)
{
	...
	while (index <= end) {
		...
		for (i = 0; i < nr_folios; i++) {
			 ...
			 */
			if (!folio_buffers(folio)) {
				ext4_warning_inode(mpd->inode, "folio %p (idx %lu) does not have buffers attached", folio, folio->index);
...
static int ext4_da_write_begin_bg(struct file *file,
				  struct address_space *mapping, loff_t pos,
				  unsigned len, struct page **pagep,
				  void **fsdata, struct dsa_emu_req_entry *req)
{
#if 0
	int ret, retries = 0;
	pgoff_t index;
	...
	*pagep = NULL;
	...
	if (ext4_nonda_switch(inode->i_sb) || ext4_verity_in_progress(inode)) {
		pr_err("ext4_nonda_switch: inode %lu has nonda switch, but bgplan is set\n",
		       inode->i_ino);
	}
	*fsdata = (void *)0;
	...
	if (ext4_test_inode_state(inode, EXT4_STATE_MAY_INLINE_DATA)) {
		pr_err("ext4_da_write_begin_bg: inode %lu has inline data, but bgplan is set\n",
		       inode->i_ino);
...
#if 0
retry:
	folio = __filemap_get_folio_bg(mapping, index, FGP_WRITEBEGIN,
				       mapping_gfp_mask(mapping),
				       &use_plan3, req);
	if (!folio && use_plan3){
		*pagep = NULL;
...
#if 0 // Reason: It is not possible for writeback to start, because it is not yet added to XArray
	if (IS_ERR(folio)) {
		return PTR_ERR(folio);
	}
...
#ifdef CONFIG_FS_ENCRYPTION
#if 0 // Reason: bgplan is always set to a value
	// We also move ext4_block_write_begin to a later stage
	if (req->bgplan & DSA_EMU_BG_PLAN_BLK_WRITE_BEGIN) {
		ret = 0;
	} else {
		if (!should_test) {
			ret = ext4_block_write_begin(folio, pos, len,
			...
			TIME_BEGIN(blk_write_begin);
			ret = ext4_block_write_begin(
				folio, pos, len, ext4_da_get_block_prep);
			TIME_END(blk_write_begin);
		}
	}
#else
	// ret = 0;
			...
			struct folio *folio)
{
	...
	if (disksize_changed) {
		...
		/* In a BG batch, owe one inode-dirty for the whole request
		 * and let ext4_bg_batch_end journal it once. */
		if (current->bg_we_defer_dirty) {
			current->bg_we_dirty_pending = true;
			return copied;
		}
...
/*
 * Bracket one BG request's write_end calls so the per-entry inode-dirty
 * journaling (i_disksize extension - every append entry) is deferred to a
 * single short-lived 2-credit handle at the end of the request, instead of
 * one handle start/stop per entry.
 *
 * Do NOT hold one handle open across the whole entry loop instead: that
 * pins t_updates for the duration of folio locking/write_end of all
 * entries on all BG workers, and fsync-driven jbd2 commits convoy behind them.
 */
static void *ext4_bg_batch_begin(struct inode *inode)
{
	current->bg_we_defer_dirty = true;
	current->bg_we_dirty_pending = false;
	return inode;
}

static void ext4_bg_batch_end(struct inode *inode, void *cookie)
{
	handle_t *handle;

	current->bg_we_defer_dirty = false;
	if (!current->bg_we_dirty_pending)
		return;
	current->bg_we_dirty_pending = false;
...
static const struct address_space_operations ext4_da_aops = {
	...
	.write_begin_bg		= ext4_da_write_begin_bg,
	.blk_write_begin	= ext4_block_write_begin_bg,
	.bg_pace_safe		= true,
	.bg_batch_begin		= ext4_bg_batch_begin,
	.bg_batch_end		= ext4_bg_batch_end,
		...
		struct address_space *mapping, loff_t from, loff_t length)
{
	...
	/* Check if BG write is pending for this page - truncation race */
	{
		struct ongoing_list_bucket *buckets =
			READ_ONCE(inode->i_ongoing_buckets);
		if (buckets) {
			int olist_state = search_ongoing_entry_state(
				folio->index, buckets);
			if (olist_state)
				pr_err("TRUNC ZERO RACE: ino=%lu index=%lu olist_state=%d from=%lld length=%lld offset=%u\n",
				       inode->i_ino,
				       (unsigned long)folio->index,
				       olist_state, from, length, offset);
		}
	}
...
int ext4_truncate(struct inode *inode)
{
	...
	UP_WRITE_TEST(inode, &ei->i_data_sem);
	if (err)
		goto out_stop;
