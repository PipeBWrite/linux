		...
		ext4_lblk_t lblk, ext4_lblk_t len, ext4_fsblk_t pblk)
{
	...
	if (!ext4_es_must_keep(es)) {
		...
		percpu_counter_add_local(&EXT4_SB(inode->i_sb)->
					s_es_stats.es_stats_shk_cnt, 1);
	}

	EXT4_I(inode)->i_es_all_nr++;
	percpu_counter_add_local(&EXT4_SB(inode->i_sb)->s_es_stats.es_stats_all_cnt, 1);
}
...
{
	EXT4_I(inode)->i_es_all_nr--;
	percpu_counter_sub_local(&EXT4_SB(inode->i_sb)->s_es_stats.es_stats_all_cnt, 1);
	...
	if (!ext4_es_must_keep(es)) {
		...
		percpu_counter_sub_local(&EXT4_SB(inode->i_sb)->
					s_es_stats.es_stats_shk_cnt, 1);
	}
...
out:
	...
	if (found) {
		...
		if (!ext4_es_is_referenced(es1))
			ext4_es_set_referenced(es1);
		percpu_counter_add_local(&stats->es_stats_cache_hits, 1);
		if (next_lblk) {
		...
		percpu_counter_add_local(&stats->es_stats_cache_misses, 1);
	}
