...
{
	if (first < second) {
		DOWN_WRITE_TEST(first, &EXT4_I(first)->i_data_sem);
		DOWN_WRITE_TEST(second, &EXT4_I(second)->i_data_sem);
...
{
	UP_WRITE_TEST(orig_inode, &EXT4_I(orig_inode)->i_data_sem);
	UP_WRITE_TEST(donor_inode, &EXT4_I(donor_inode)->i_data_sem);
}
