				...
				bool should_test)
{
	...
	while (!list_empty(&wb->b_io)) {
		...
		write_chunk = writeback_chunk_size(wb, work, should_test);
				  ...
				  bool should_test)
{
	...
	while (!list_empty(&wb->b_io)) {
		...
		wrote += writeback_sb_inodes(sb, wb, work, should_test);
		up_read(&sb->s_umount);
				...
				enum wb_reason reason)
{
	...
	__writeback_inodes_wb(wb, &work, false);
	spin_unlock(&wb->list_lock);
...
long wb_writeback(struct bdi_writeback *wb,
			 struct wb_writeback_work *work, bool should_test)
{
	...
	for (;;) {
		...
		if (work->sb)
			...
			progress = __writeback_inodes_wb(wb, work, should_test);

		// if (should_test) {
		// 	pr_info("Written %lu pages, work->sb non-null? %d\n",
		// 		progress, work->sb == NULL);
		// }
...
static long wb_check_background_flush(struct bdi_writeback *wb)
{
	if (wb_over_bg_thresh(wb)) {
		...
		return wb_writeback(wb, &work, false);
	}
...
static long wb_check_start_all(struct bdi_writeback *wb)
{
	...
	if (nr_pages) {
		...
		nr_pages = wb_writeback(wb, &work, false);
	}
...
static long wb_do_writeback(struct bdi_writeback *wb)
{
	...
	while ((work = get_next_work_item(wb)) != NULL) {
		...
		wrote += wb_writeback(wb, work, false);
		finish_writeback_work(wb, work);
	}
...
void wb_workfn(struct work_struct *work)
{
	...
	long pages_written;
	// pr_info("%s: wb_workfn created on cpu %d\n", __func__, smp_processor_id());
