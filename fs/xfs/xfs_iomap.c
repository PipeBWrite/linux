...
/*
 * Background buffered writes must not call iomap_end (punch_delalloc).
 * The FG path already allocated the delalloc reservation and memcpy'd data
 * into the folio.  BG only does metadata updates (write_begin/write_end).
 * Calling punch_delalloc in BG on stale iomap retries causes a deadlock:
 * punch_delalloc -> mapping_seek_hole_data -> folio_wait_bit on a folio
 * held by another pending BG entry in the same single-threaded BG queue.
 */
const struct iomap_ops xfs_buffered_write_iomap_bg_ops = {
	.iomap_begin		= xfs_buffered_write_iomap_begin,
};
