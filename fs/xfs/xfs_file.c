...
xfs_file_fsync(
	...
	int			datasync)
{
	return __xfs_file_fsync(file, start, end, datasync);
}
...
write_retry:
	...
	if (filemap_bg_compatible(inode) && !dsa_emu_should_use_orig_fallback())
		ret = xfs_iomap_file_buffered_write_bg(iocb, from);
	else {
