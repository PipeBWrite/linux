	...
	xfs_ino_t		ino)
{
	...
	VFS_I(ip)->i_ongoing_buckets =
		kcalloc(ONGOING_HASH_NUM,
			sizeof(*VFS_I(ip)->i_ongoing_buckets),
			GFP_KERNEL);
	atomic_set(&VFS_I(ip)->i_draining, 0);
	VFS_I(ip)->i_snapshot_counter = DSA_EMU_BG_SNAPSHOT_COUNTER;
	VFS_I(ip)->i_bg_ongoing_snapshot = 0;
	for (int i = 0; i < ONGOING_HASH_NUM; i++)
		INIT_HLIST_HEAD(&VFS_I(ip)->i_ongoing_buckets[i].list);
	...
	struct xfs_inode	*ip)
{
	...
	call_rcu(&inode->i_rcu, xfs_inode_free_callback);
}
