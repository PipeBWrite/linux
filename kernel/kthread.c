...
int kthread_worker_fn_without_schedule(void *worker_ptr)
{
	struct kthread_worker *worker = worker_ptr;
	struct kthread_work *work;
...
EXPORT_SYMBOL_GPL(kthread_worker_fn_without_schedule);
...
static __printf(3, 0) struct kthread_worker *
__kthread_create_worker_without_schedule(int cpu, unsigned int flags,
			const char namefmt[], va_list args)
{
	struct kthread_worker *worker;
	struct task_struct *task;
	...
	task = __kthread_create_on_node(kthread_worker_fn_without_schedule, worker,
						node, namefmt, args);
	if (IS_ERR(task))
		goto fail_task;
...
struct kthread_worker *
kthread_create_worker_on_cpu_without_schedule(int cpu, unsigned int flags,
			     const char namefmt[], ...)
{
	struct kthread_worker *worker;
	va_list args;
	...
	va_start(args, namefmt);
	worker = __kthread_create_worker_without_schedule(cpu, flags, namefmt, args);
	va_end(args);

	return worker;
}
EXPORT_SYMBOL(kthread_create_worker_on_cpu_without_schedule);
