 ...
 * lock for reading
 */
void __sched down_read_test(struct rw_semaphore *sem, struct inode *inode)
{
	might_sleep();
	rwsem_acquire_read(&sem->dep_map, 0, 0, _RET_IP_);
 ...
 * lock for writing
 */
void __sched down_write_test(struct rw_semaphore *sem, struct inode *inode)
{
	might_sleep();
	rwsem_acquire(&sem->dep_map, 0, 0, _RET_IP_);
	LOCK_CONTENDED(sem, __down_write_trylock, __down_write);
}
EXPORT_SYMBOL(down_write_test);
 ...
 * release a read lock
 */
void up_read_test(struct rw_semaphore *sem)
{
	rwsem_release(&sem->dep_map, _RET_IP_);
	__up_read(sem);
}
EXPORT_SYMBOL(up_read_test);

/*
 * release a write lock
 */
void up_write_test(struct rw_semaphore *sem)
{
	rwsem_release(&sem->dep_map, _RET_IP_);
	__up_write(sem);
}
EXPORT_SYMBOL(up_write_test);
