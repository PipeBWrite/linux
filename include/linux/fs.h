...
struct address_space_operations {
	...
	int (*write_begin_bg)(struct file *, struct address_space *mapping,
				loff_t pos, unsigned len,
				struct page **pagep, void **fsdata,
				struct dsa_emu_req_entry *req);
	int (*blk_write_begin)(struct folio *, loff_t pos, unsigned len);
	/*
	 * True if adaptive bg-fsync pacing (debug_bg_fsync=auto) is safe on
	 * this write path.  Pacing overlaps writeback with in-flight BG/FG
	 * writes to the same inode; that is only acceptable when writers
	 * take the inode mapping lock shared (ext4: i_data_sem read + es
	 * tree, so the paced flusher's down_write absorbs the contention).
	 * Filesystems whose buffered-write path takes an exclusive per-inode
	 * lock per entry (xfs: ILOCK_EXCL in xfs_buffered_write_iomap_begin)
	 * convoy behind the paced flusher's shared mapping walks and must
	 * leave this clear.
	 */
	bool bg_pace_safe;
	/*
	 * Optional: bracket one BG request's write_end calls on an inode so
	 * the filesystem can share per-batch state (ext4: one jbd2 handle
	 * for the whole request instead of one per entry).  Returns an
	 * opaque cookie or NULL.
	 */
	void *(*bg_batch_begin)(struct inode *inode);
	void (*bg_batch_end)(struct inode *inode, void *cookie);
 ...
 */
struct address_space {
	/* --- cacheline 0: read-mostly, hot in FG write path --- */
	struct inode		*host;
	const struct address_space_operations *a_ops;
	gfp_t			gfp_mask;

	atomic_t		i_mmap_writable ____cacheline_aligned_in_smp;
	struct rb_root_cached	i_mmap;
	errseq_t		wb_err;
	void *			i_private_data;

	/* --- cacheline 1: write-heavy, modified by BG/writeback --- */
	struct xarray		i_pages ____cacheline_aligned_in_smp;
	...
	/* --- cacheline 2+: locks --- */
	struct rw_semaphore	invalidate_lock ____cacheline_aligned_in_smp;
...
/* Per-bucket ongoing list, cacheline-aligned to limit false sharing. */
struct ongoing_list_bucket {
...
struct inode {
	...
	struct ongoing_list_bucket *i_ongoing_buckets; /* Ongoing req buckets */
	...
	dev_t			i_rdev;
	loff_t			i_size ____cacheline_aligned_in_smp;
	...
	unsigned long		dirtied_when ____cacheline_aligned_in_smp;
	...
	atomic_t		i_bg_pending ____cacheline_aligned_in_smp;
	spinlock_t		i_shift_index_lock;
	atomic_t		i_bg_fsync_lock;
	struct completion	i_bg_drained;
	...
	struct filemap_bg_inode_batch *i_fg_batch ____cacheline_aligned_in_smp;
	atomic_t		i_draining;
	struct rw_semaphore	i_rwsem;
	int				i_snapshot_counter;
	int             i_bg_ongoing_snapshot;

#ifdef CONFIG_SCACHE_MEMORY
	struct sc_per_file_cache i_sc_cache ____cacheline_aligned_in_smp;
#endif
};
...
struct file {
	...
	u64				f_est_dirty_count;
	u64			f_bg_writes_since_fsync;
	u64			f_bg_fsync_recovery_writes;
	bool			f_bg_fsync_fallback;
} __randomize_layout
...
ssize_t generic_perform_write_bg(struct kiocb *, struct iov_iter *, bool large_folio_support);
ssize_t generic_perform_write_bg_cb(struct kiocb *, struct iov_iter *,
		dsa_emu_write_entry_fn_t write_fn, bool large_folio_support);
ssize_t generic_perform_write_bg_slow(struct kiocb *, struct iov_iter *);
