...
noinline pgprot_t vm_get_page_prot_dsa(void)
{
	unsigned long val = pgprot_val(PAGE_KERNEL_DSA);
...
EXPORT_SYMBOL(vm_get_page_prot_dsa);

noinline pgprot_t vm_get_page_prot(unsigned long vm_flags)
{
